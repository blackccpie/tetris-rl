...
- **Reward**: Hybrid PBRS `tetris_env.py:206` `score_delta + α*shaped_delta` where `shaped = get_total_score()` (`-0.5 height +0.75 lines -0.35 holes -0.2 bump` `tetris_env.py:245`, GA-tuned donny `9.9/-9.25/-1.43` alt). `α=0.1` default (`--shaped-alpha 0.0` legacy pure score, sparse → plateau `Score 5-9`). Preserves optimal policy (Ng 1999).
...
## Key Files
- `tetris_env.py` — Gymnasium Env subclass; 7 actions, observation (18,10) uint8, reward = hybrid `score + α*shaped` (`shaped_alpha`).
